...
static inline int low_idle_policy(int policy)
{
...
{
	return idle_policy(policy) || fair_policy(policy) || rt_policy(policy) || dl_policy(policy) || low_idle_policy(policy);
}
 ...
 */
static inline int _double_lock_balance(struct rq *this_rq, struct rq *busiest) __releases(this_rq->lock) __acquires(busiest->lock)
