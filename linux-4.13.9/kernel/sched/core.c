...
void rt_mutex_setprio(struct task_struct *p, struct task_struct *pi_task)
{
	 ...
	 */
	if (dl_prio(prio)) {
		if (!dl_prio(p->normal_prio) || (pi_task && dl_entity_preempt(&pi_task->dl, &p->dl))) {
			p->dl.dl_boosted = 1;
...
recheck:
	...
	if (user && !capable(CAP_SYS_NICE)) {
		...
		if (low_idle_policy(p->policy) && !low_idle_policy(policy)) {
			if (!can_nice(p, task_nice(p)))
				return -EPERM;
		}
...
SYSCALL_DEFINE4(sched_getattr, pid_t, pid, struct sched_attr __user *, uattr, unsigned int, size, unsigned int, flags)
{
	...
	if (!uattr || pid < 0 || size > PAGE_SIZE || size < SCHED_ATTR_SIZE_VER0 || flags)
		return -EINVAL;
...
void sched_show_task(struct task_struct *p)
{
	...
	if (state)
		state = __ffs(state) + 1;
	printk(KERN_INFO "%-15.15s %c", p->comm, state < sizeof(stat_nam) - 1 ? stat_nam[state] : '?');
...
void ___might_sleep(const char *file, int line, int preempt_offset)
{
	...
	printk(KERN_ERR "in_atomic(): %d, irqs_disabled(): %d, pid: %d, name: %s\n", in_atomic(), irqs_disabled(), current->pid,
	       current->comm);
