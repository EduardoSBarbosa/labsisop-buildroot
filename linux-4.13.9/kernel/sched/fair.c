...
static inline void update_stats_wait_end(struct cfs_rq *cfs_rq, struct sched_entity *se)
{
	...
	schedstat_set(se->statistics.wait_max, max(schedstat_val(se->statistics.wait_max), delta));
...
static inline unsigned long group_faults(struct task_struct *p, int nid)
{
	...
	return p->numa_group->faults[task_faults_idx(NUMA_MEM, nid, 0)] + p->numa_group->faults[task_faults_idx(NUMA_MEM, nid, 1)];
}
...
static void update_numa_stats(struct numa_stats *ns, int nid)
{
	...
	ns->task_capacity = min_t(unsigned, capacity, DIV_ROUND_CLOSEST(ns->compute_capacity, SCHED_CAPACITY_SCALE));
...
static bool load_too_imbalanced(long src_load, long dst_load, struct task_numa_env *env)
{
	...
	imb = dst_load * src_capacity * 100 - src_load * dst_capacity * env->imbalance_pct;
	if (imb <= 0)
		return false;
	...
	old_imb = orig_dst_load * src_capacity * 100 - orig_src_load * dst_capacity * env->imbalance_pct;
...
static void task_numa_compare(struct task_numa_env *env, long taskimp, long groupimp)
{
	...
	if (!cur) {
		...
		if (env->src_stats.nr_running <= env->src_stats.task_capacity && !env->dst_stats.has_free_capacity)
			goto unlock;
 ...
 */
static inline bool numa_wake_affine(struct sched_domain *sd, struct task_struct *p, int this_cpu, int prev_cpu, int sync)
{
 ...
 *                     n=1
 */
static __always_inline u32 accumulate_sum(u64 delta, int cpu, struct sched_avg *sa, unsigned long weight, int running,
					  struct cfs_rq *cfs_rq)
{
	...
	if (periods) {
		...
		if (cfs_rq) {
			cfs_rq->runnable_load_sum = decay_load(cfs_rq->runnable_load_sum, periods);
		}
 ...
 */
static __always_inline int ___update_load_avg(u64 now, int cpu, struct sched_avg *sa, unsigned long weight, int running,
					      struct cfs_rq *cfs_rq)
{
	u64 delta;
...
static int __update_load_avg_blocked_se(u64 now, int cpu, struct sched_entity *se)
{
	return ___update_load_avg(now, cpu, &se->avg, 0, 0, NULL);
}

static int __update_load_avg_se(u64 now, int cpu, struct cfs_rq *cfs_rq, struct sched_entity *se)
{
	return ___update_load_avg(now, cpu, &se->avg, se->on_rq * scale_load_down(se->load.weight), cfs_rq->curr == se, NULL);
}

static int __update_load_avg_cfs_rq(u64 now, int cpu, struct cfs_rq *cfs_rq)
{
	return ___update_load_avg(now, cpu, &cfs_rq->avg, scale_load_down(cfs_rq->load.weight), cfs_rq->curr != NULL, cfs_rq);
}
...
static inline void update_tg_cfs_util(struct cfs_rq *cfs_rq, struct sched_entity *se)
{
...
static inline void enqueue_entity_load_avg(struct cfs_rq *cfs_rq, struct sched_entity *se)
{
	struct sched_avg *sa = &se->avg;
...
static inline void dequeue_entity_load_avg(struct cfs_rq *cfs_rq, struct sched_entity *se)
{
	cfs_rq->runnable_load_avg = max_t(long, cfs_rq->runnable_load_avg - se->avg.load_avg, 0);
	cfs_rq->runnable_load_sum = max_t(s64, cfs_rq->runnable_load_sum - se->avg.load_sum, 0);
}
...
static void set_next_entity(struct cfs_rq *cfs_rq, struct sched_entity *se)
{
	...
	if (schedstat_enabled() && rq_of(cfs_rq)->load.weight >= 2 * se->load.weight) {
		schedstat_set(se->statistics.slice_max,
			      max((u64)schedstat_val(se->statistics.slice_max), se->sum_exec_runtime - se->prev_sum_exec_runtime));
	}
...
static void __return_cfs_rq_runtime(struct cfs_rq *cfs_rq)
{
	...
	raw_spin_lock(&cfs_b->lock);
	if (cfs_b->quota != RUNTIME_INF && cfs_rq->runtime_expires == cfs_b->runtime_expires) {
...
static const u8 degrade_factor[CPU_LOAD_IDX_MAX][DEGRADE_SHIFT + 1] = { { 0, 0, 0, 0, 0, 0, 0, 0 },
									{ 64, 32, 8, 0, 0, 0, 0, 0 },
									{ 96, 72, 40, 12, 1, 0, 0, 0 },
...
static inline void cpu_load_update_nohz(struct rq *this_rq, unsigned long curr_jiffies, unsigned long load)
{
}
#endif /* CONFIG_NO_HZ_COMMON */
...
static int wake_affine(struct sched_domain *sd, struct task_struct *p, int prev_cpu, int sync)
{
...
static struct sched_group *find_idlest_group(struct sched_domain *sd, struct task_struct *p, int this_cpu, int sd_flag)
{
	...
	do {
		...
		if (!cpumask_intersects(sched_group_span(group), &p->cpus_allowed))
			continue;
 ...
 */
static inline bool group_smaller_cpu_capacity(struct sched_group *sg, struct sched_group *ref)
{
	return sg->sgc->min_capacity * capacity_margin < ref->sgc->min_capacity * 1024;
}
 ...
 */
static inline void update_sg_lb_stats(struct lb_env *env, struct sched_group *group, int load_idx, int local_group,
...
static bool update_sd_pick_busiest(struct lb_env *env, struct sd_lb_stats *sds, struct sched_group *sg, struct sg_lb_stats *sgs)
{
	 ...
	 */
	if (sgs->sum_nr_running <= sgs->group_weight && group_smaller_cpu_capacity(sds->local, sg))
		return false;
...
static inline void update_sd_lb_stats(struct lb_env *env, struct sd_lb_stats *sds)
{
	...
	do {
		 ...
		 */
		if (prefer_sibling && sds->local && group_has_capacity(env, local) &&
...
static int check_asym_packing(struct lb_env *env, struct sd_lb_stats *sds)
{
	...
	env->imbalance = DIV_ROUND_CLOSEST(sds->busiest_stat.avg_load * sds->busiest_stat.group_capacity, SCHED_CAPACITY_SCALE);
...
static inline void calculate_imbalance(struct lb_env *env, struct sd_lb_stats *sds)
{
	...
	env->imbalance = min(max_pull * busiest->group_capacity, (sds->avg_load - local->avg_load) * local->group_capacity) /
...
static struct sched_group *find_busiest_group(struct lb_env *env)
{
	...
	if (env->idle == CPU_NEWLY_IDLE && group_has_capacity(env, local) && busiest->group_no_capacity)
		goto force_balance;
...
static struct rq *find_busiest_queue(struct lb_env *env, struct sched_group *group)
{
	...
	for_each_cpu_and (i, sched_group_span(group), env->cpus) {
		...
		if (rq->nr_running == 1 && wl > env->imbalance && !check_cpu_capacity(rq, env->sd))
			continue;
...
static int need_active_balance(struct lb_env *env)
{
	...
	if (env->idle == CPU_NEWLY_IDLE) {
		 ...
		 */
		if ((sd->flags & SD_ASYM_PACKING) && sched_asym_prefer(env->dst_cpu, env->src_cpu))
			return 1;
	}
	...
	*/
	...
	if (sd) {
		for_each_cpu (i, sched_domain_span(sd)) {
			if (i == cpu || !cpumask_test_cpu(i, nohz.idle_cpus_mask))
				continue;
